...
class SignalComponentGeneration:
    ...
    def generate_signal_components(self):
        ...
        out_yaml_dict = {'a. Desired number of repertoires': self.desired_num_repertoires,
    ...
    def _determine_signal_sequence_combination(self, pgen_intervals_array, pool_size):
        if len(pgen_intervals_array) > pool_size:
            ...
            if condition_met:
                ...
                sequence_proportion, implantation_count = max(subset_seqs_total_implant_counts.items(),
                                                              key=lambda k: k[1])
                implantable_seq_subset_indices = subset_seq_indices[sequence_proportion]
        else:
    ...
    def get_signal_seq_combination(self, pgen_intervals_array):
        ...
        if condition_met:
            ...
            obtained_pool_size, implantation_stats = max(optimal_implantation_stats.items(), key=lambda x: x[1][1])
